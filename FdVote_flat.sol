...
contract FdVote is Context{
     ...
     * 投票付款
     */
    function voteToken(uint256 _userId, uint256 _orderId, uint256 _paidNum) public returns(bool){
        ...
        if(orders[_orderId].isExist == true){
            (,uint256 _limitAmount)= SafeMath.tryAdd(orders[_orderId].amount,_paidNum);
            require(_limitAmount <= limitAmount, "maxNumber error");
        ...
        token.safeTransferFrom(_msgSender(),profitor,_paidNum);
     ...
     * 投票提取
     */
    function voteWidth(uint256 _userId, uint256 _orderId) public returns(bool){
        require(address(_msgSender()) == address(tx.origin), "no contract");
    ...
    function setLimitAmount(uint256 _limitAmount) public onlyOwner () {
        require(_limitAmount > 0 ,"_limitAmount is not zero");
