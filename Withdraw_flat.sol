...
contract Withdraw is Context{
    ...
    modifier onlyProfitor() {
        require(profitor == _msgSender(), "Ownable: caller is not the profitor");
        _;
    }
     ...
     * 质押函数
     */
    function withdrawToken(uint256 _userId, uint256 _orderId, uint256 _paidNum) payable public {
        require(address(_msgSender()) == address(tx.origin), "no contract");
        require(_paidNum > 0, "minNumber error");
    ...
    function withdrawStatus(address _username, uint256 _orderId, IERC20 _orderPaid) public onlyOwner returns(bool){
        ...
        IERC20 token = _orderPaid;
        require(tokens[address(token)] == true, "orderPaid error");
        require(token.allowance(profitor,address(this)) >= orders[_orderId].number, "token no profitor allowance");
        token.safeTransferFrom(profitor,_username,orders[_orderId].number);
        orders[_orderId].status = true;
        return true;
    }
     ...
     * 设置分配者账号地址
     */
    function setProfitor(address _profitor) public onlyProfitor () {
        require(_profitor != address(0),"_profitor is not zero address");
        profitor = _profitor;
    }

    function getTokens(IERC20 _token) public view returns(bool){

       return tokens[address(_token)];
    }
