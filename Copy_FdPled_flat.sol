...
contract FdPled is Context{
    ...
    mapping(uint256 => keyFlag) public orders;
    ...
    function swapToken(uint256 _userId, uint256 _orderId, address _parent, uint256 _paidNum) public returns(uint256){
        ...
        uint256 _swapNum =  swapGetAmountOut(_paidNum);
        swapTokensForTokens(_paidNum);
        uint256 _balanNum = daibi.balanceOf(address(this));
        _swapNum = _swapNum >= _balanNum ? _balanNum : _swapNum;
        keys.push(keyFlag(_msgSender(),true,_orderId,_userId,_swapNum));
        uint256 key =  keys.length - 1;
        orders[_orderId] = keys[key];
    ...
    //最终调用uniswapV2Router的swapExactTokensForETHSupportingFeeOnTransferTokens方法执行兑换。
    ...
    function swapTokensForTokens(uint256 tokenAmount) private{
        ...
        //path[1] = 0x93212Ed63A142F5E3319C404Fc3cc01779f84d42;
    ...
    function swapGetAmountOut(uint256 tokenAmount) private view returns(uint){
        (uint112 reserve0, uint112 reserve1,) = IUniswapV2Pair(uniswapV2Pair).getReserves();
        // make the swap
        return uniswapV2Router.getAmountOut(tokenAmount,reserve0,reserve1);
    }

    function getSwapAmountOut(uint256 tokenAmount) public  view returns(uint){
        return swapGetAmountOut(tokenAmount);
    }

    /**
     * 检查订单
     */
    function checkOrderExists(uint256 _userId, uint256 _orderId) public view returns(bool){
        keyFlag storage order = orders[_orderId];
        if(orders[_orderId].userId != _userId){
            return false;
        }
        if(order.isExist == true){
            return true;
        }
        return false;
    }
    ...
    function getMinNumber() public view returns(uint256){
       return minNumber;
    }
